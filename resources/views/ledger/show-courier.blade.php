...
@endpush
@section('content')
<div id="courier-ledger-page" class="p-3">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <div class="d-flex mb-4">
                    <h5 class="align-self-center page-title">Ledger Book: {{ $courier->name }}</h5>
                    
                    @can('viewAny', App\CourierLedger::class)
                    <div class="ml-auto" v-cloak>
                        <div class="input-group mb-3">
                            ...
                            data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-filter pr-2"></i> {{ $courier->name ?? 'Select Store' }} </button>
                            <div class="dropdown-menu">
                                <a v-for="courier in couriers" class="dropdown-item" v-bind:href="courier.ledger_url">@{{ courier.name }}</a></option>
                            </div>
                        </div>
                    </div>
                    @endcan
                   ...
                   @if(Auth::user()->hasRole('courier'))
                    <div class="ml-auto align-self-center">
                        <div class="text-center">
                            @if( ! $courier->meta->payment_requested_at && $data->balance >= $courier->meta->credit_limit)
                            ...
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-start">
                                    <div>
                                        ...
                                        <div>
                                            <h4><strong class="text-ink"><small>NRs.</small> {{ number_format($courier->meta->credit_limit) }}  </strong></h4>
                                        </div>
                                    </div>
                                    <div class="grey lighten-4 rounded p-2">
                    ...
                    <div class="card-header grey lighten-5">
                        <div class="d-flex">
                            <div class="align-self-center">
                                Showing ledger of {{ $courier->name }}
                            </div>

                            @can('create', App\CourierLedger::class)
                            @if($data->balance >= $courier->meta->credit_limit)
                            <div class="ml-auto">
                                <div class="text-center">
...
<div class="modal fade" id="modalDepositForm" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('courier_ledgers.store', $courier->id) }}" method="POST" class="form white">
                @csrf
...
<div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
        ...
        <form action="{{ route('courier.payment.request', $courier) }}" method="POST" class="form">
            @csrf
...
<script>
    $(function () {
        ...
        var adminSidebar = new Vue({
            el: '#courier-ledger-page',
            data: {
                couriers: []
            },
            created() {
                this.fetchCouriersList();
            },
            methods: {
                fetchCouriersList: function(){
                    axios.get("{{ route('ajax.courier-ledgers.couriers_list') }}")
                    .then(response => {
        ...
        var printTitle = '<span style="font-size: 20px; font-weight: 600;">Chhitomitho.com</span><small style="font-size: 12px; float:right;">Date: ' + today + '</small><p style="font-size: 14px;">Report of:  {{ $courier->name }}</p>';
        var exportTitle = '{{ $courier->name }} ( ' + today + ' )';
